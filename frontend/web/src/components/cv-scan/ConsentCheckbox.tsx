  ...
  return (
    <div
      className={cn(
        ...
        checked ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 bg-gray-50',
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div className={cn(
          'p-2 rounded-full',
          checked ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-200 text-gray-500'
        )}>
      ...
      <label
        className={cn(
          ...
          checked ? 'border-emerald-300 bg-white' : 'border-gray-200 bg-white hover:bg-gray-50',
      ...
      {showRedactionOption && checked && (
        <div className="border-t pt-4">
          <label
            className={cn(
              'flex items-start gap-3 p-3 rounded-md border cursor-pointer transition-colors',
              redactPII ? 'border-amber-300 bg-amber-50' : 'border-gray-200 bg-white hover:bg-gray-50'
            )}
          >
            <input
              type="checkbox"
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-1">
                {t('redactPIIDescription') || 'Remove name, email, and phone before AI analysis'}
              </p>
            </div>
          </label>
        </div>
      ...
      <div className="flex items-start gap-2 text-xs text-gray-500 bg-gray-100 p-2 rounded">
        ...
        <p>
          {t('privacyNotice') || 'Your data is processed securely and never shared without your explicit permission.'}
        </p>
      </div>
      ...
      {!checked && (
        <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 p-2 rounded border border-amber-200">
          ...
          <p>
            {t('noConsentWarning') || 'Without consent, only basic parsing will be performed. AI-powered suggestions will not be available.'}
          </p>
        </div>
      )}
    </div>

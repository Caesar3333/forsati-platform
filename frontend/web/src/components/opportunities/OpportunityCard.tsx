...
export default function OpportunityCard({ opportunity, type, locale }: OpportunityCardProps) {
  ...
  return (
    ...
    >
        ...
        <div className="mt-4 space-y-2">
          ...
          {opportunity.deadline && (
            <div className="flex items-center gap-2 text-sm text-red-600 dark:text-red-400">
              ...
              {new Date(opportunity.deadline).toLocaleDateString(locale === 'ar' ? 'ar-JO' : 'en-JO')}
            </div>
          )}
        </div>
      ...
      <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
        {type === 'course' && (
          <span className={`text-sm font-medium ${opportunity.isFree ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-600 dark:text-gray-400'}`}>
            {opportunity.isFree ? (locale === 'ar' ? 'مجاني' : 'Free') : (locale === 'ar' ? 'مدفوع' : 'Paid')}
          </span>
        )}

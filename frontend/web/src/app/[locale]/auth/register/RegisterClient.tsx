...
export function RegisterClient({ locale, callbackUrl, defaultType }: RegisterClientProps) {
  const t = useTranslations('auth.register');
  const [isLoading, setIsLoading] = useState(false);
  const [accountType, setAccountType] = useState<'jobseeker' | 'employer'>(defaultType);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {errors.fullName && (
                <p className="mt-1 text-sm text-red-500">{t('errors.invalidName')}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.password && (
                <p className="mt-1 text-sm text-red-500">{t('errors.weakPassword')}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.confirmPassword && (
                <p className="mt-1 text-sm text-red-500">{t('errors.passwordMismatch')}</p>
              )}
            </div>
            ...
            {errors.acceptTerms && (
              <p className="text-sm text-red-500">{t('errors.mustAcceptTerms')}</p>
            )}
          </div>

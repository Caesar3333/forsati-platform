...
export default function JobCard({
  ...
  const formatSalary = (salary: Job["salary"]) => {
    ...
    if (salary.max)
      return t("salaryUpTo", { amount: formatter.format(salary.max) });
    return null;
  };
  ...
  const formatDate = (dateStr: string) => {
    ...
    if (diffDays < 30)
      return t("postedWeeksAgo", { weeks: Math.floor(diffDays / 7) });
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
        ...
        {/* Job Type */}
        <span
          className={`px-2 py-0.5 rounded-full text-xs font-medium ${typeColors[job.type]}`}
        >
          {typeLabels[job.type]}
        </span>

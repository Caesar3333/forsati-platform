...
function mapSkillLevel(level?: string): SkillLevel {
  ...
  if (normalized.includes('expert') || normalized.includes('advanced') || normalized.includes('senior')) {
    return 'expert';
  }
  if (normalized.includes('proficient') || normalized.includes('good')) {
    return 'advanced';
  }
  if (normalized.includes('beginner') || normalized.includes('basic') || normalized.includes('learning')) {
    return 'beginner';
  }
...
function detectEmploymentStatus(parsed: ParsedResume): EmploymentStatus {
  ...
  const currentExp = parsed.experience?.find(e => 
    !e.endDate || e.endDate.toLowerCase().includes('present') || e.endDate.toLowerCase().includes('current')
  );
  ...
  if (summary.includes('recent graduate') || summary.includes('fresh graduate') || summary.includes('خريج جديد')) {
    return 'new_graduate';
  }
  
  // Check for freelance indicators
  if (summary.includes('freelance') || summary.includes('consultant') || summary.includes('مستقل')) {
    return 'freelance';
  }
  
  // Check for seeking indicators
  if (summary.includes('seeking') || summary.includes('looking for') || summary.includes('أبحث عن')) {
    return 'seeking';
  }
 ...
 */
function extractLinkedInUrl(profiles?: ParsedResume['basics']['profiles']): string | undefined {
...
function generateHeadline(parsed: ParsedResume): string {
  const currentExp = parsed.experience?.find(e => 
    !e.endDate || e.endDate.toLowerCase().includes('present')
  );
    ...
    for (const edu of parsed.education) {
      if (edu.institution) {
        education.push({
          ...
          field: edu.area || '',
          start_year: extractYear(edu.startDate) || new Date().getFullYear() - 4,
    ...
    for (const proj of parsed.projects) {
      if (proj.name) {
        portfolio_projects.push({
          ...
          role: '',
          year: extractYear(proj.endDate) || extractYear(proj.startDate) || new Date().getFullYear(),
          media_urls: [],
          repo_url: '',
          live_url: proj.url || '',
...
export function mergeProfiles(
  ...
  const simpleFields: (keyof UserProfile)[] = [
    'full_name', 'headline', 'summary', 'employment_status',
    'primary_location', 'email', 'phone', 'linkedin_url', 'website_url',
  ...
  const arrayFields: (keyof UserProfile)[] = [
    ...
    'portfolio_projects', 'languages', 'preferred_locations',
  ];
        ...
        for (const item of importedArr) {
          ...
          const isDuplicate = merged.some(existing => {
            ...
            if (field === 'experiences') {
              return existing.company === item.company && existing.title === item.title;
            }
            if (field === 'education') {
              return existing.institution === item.institution && existing.degree === item.degree;
            }
            if (field === 'certifications') {
              return existing.name === item.name && existing.issuer === item.issuer;
            }
